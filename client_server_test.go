// Copyright 2013 The dvonthenen WebSocket Authors. All rights reserved.
...
func TestDialCookieJar(t *testing.T) {
	...
	cookies := []*http.Cookie{{Name: "dvonthenen", Value: "ws", Path: "/"}}
	...
	var dvonthenen string
	var sessionID string
	for _, c := range d.Jar.Cookies(u) {
		if c.Name == "dvonthenen" {
			dvonthenen = c.Value
		}
